...
<html><head>
	...
	<title><?php 
		if(is_404()){
			echo get_bloginfo("name") . ' - 404';
		} else {
			echo get_bloginfo("name") . ' - ' . get_page_by_path(get_page_uri(), 'OBJECT', array('post', 'page'))->post_title; 
		}
	?></title>
	<?php wp_head() ?>
	<style type="text/css">
...
<body  <?php body_class() ?> style="max-width:1440px;margin:auto;<?php 
	$pagebgcolor = get_theme_mod('itpharma2_page_background');
	if($pagebgcolor){ echo 'background-color: '.$pagebgcolor.';'; }
?>">
<div class="cropbody">
	<?php 
	...
	?>" aria-label="<?php esc_attr_e( 'Основное меню', 'itpharma2' ); ?>">
		<a class="navbar-brand" id="navbar-brand_primary" href="/"><?php 
